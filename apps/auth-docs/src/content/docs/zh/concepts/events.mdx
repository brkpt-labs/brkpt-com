...
**命令（Commands）**，命名空间指明了应该处理该事件的模块：

- `brkpt-auth.session.revoke` — `session` 模块应该撤销这个会话
- `brkpt-auth.session.revoke-others` — `session` 模块应该撤销除此之外的所有会话
- `brkpt-auth.session.validate` — `session` 模块应该确认这个会话是否仍然有效
- `brkpt-auth.verification.send` — 匹配到的验证模块应该发送一个凭证
- `brkpt-auth.verification.verify` — 匹配到的验证模块应该校验一个凭证
