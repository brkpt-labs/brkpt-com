...
**Commands**, where the namespace names the module that should act on the event:

- `brkpt-auth.session.revoke` — the `session` module should revoke this session
- `brkpt-auth.session.revoke-others` — the `session` module should revoke every other session
- `brkpt-auth.session.validate` — the `session` module should confirm this session is still valid
- `brkpt-auth.verification.send` — whichever verification module matches should send a proof
- `brkpt-auth.verification.verify` — whichever verification module matches should check a proof
