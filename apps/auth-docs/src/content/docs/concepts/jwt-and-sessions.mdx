...
Sessions are stored in a fast external store like Redis rather than the primary user table, for performance. Refresh tokens are not rotated.

This differs from both the classic server-side `@session` cookie model and the reissue-and-invalidate pattern used by many OAuth-based providers. A persistent session record is the source of truth: both tokens issued at sign-in carry nothing but a pointer to it (`sid`). The session record, not the token, is what gets revoked, inspected, and managed, directly by `sid`.
